...
【通用实体提取规则（右半页必须包含 4 个独立的 JSON 文本块！）】：

一、右半页 (bbox_rel.left 设为 0.52)：
1. 【证书标题 (Title)】（必须作为独立的第一个文本块输出！）：
   - 认读原图右页最顶部的完整中文标题，逐字据实直译：
   ...
   - 【严禁事项】：绝对不能遗漏标题！绝对不能把标题和下面的正文拼在同一个文本块里！

2. 【正文标准长句 (Main Body)】：
   - 提取学生姓名、性别、出生年月/年龄、籍贯、学校名称、修业年限、考核结果、准予毕业等全部正文信息。
   - 【100% 缝合成唯一一条语法流畅的标准英文公证长句】，并用 `<b>...</b>` 标签包裹实体。
...
- 提取学籍号、毕业证号、验证印章说明、学校行政公章说明、钢印说明等。
...
"""
        ...
        try:
            ...
            messages = [
                {"role": "system", "content": "你是一个严格据实逐字直译证书全称标题、绝对独立提取标题块的通用视觉公证翻译助手。"},
